...
def gen_rectangle_vertices(x_offset):  
    # print(x_offset)
    ...
    for level in range(levels):
        ...
        xs = np.arange(x_offset-736/2, x_offset+736/2, draw_every_x)
        print(x_offset-736/2, x_offset+736/2)
        # quit()
...
def make_box_counting(swarm, video_file, trajectory_file):
    ...
    rect_objs = gen_rectangle_vertices(center_x)
    ...
    output_video = cv2.VideoWriter(f'Cropped_Swarm{swarm}_Trial{trial}_Box_Counting.avi', fourcc, 15, (912, 736))
    ...
    # Create a blank white frame
        ...
        top_left = (int(center_x - 736/2), 0)
        bottom_right = (int(736/2 +  center_x), 736)
        cv2.rectangle(overlay, top_left, bottom_right, (0, 200, 0), -1)   
...
for swarm in [1, 2, 3, 4, 5, 6, 7, 8]:
    ...
    for trial in [1, 2, 3]:
        ...
        video_file = f'initial_in_vivo_swarms_fixed_CSV\Swarm_{swarm}\Swarm{swarm}_Trial{trial}\\Swarm{swarm}_Trial{trial}.{file_ending}'
