...
#evolved swarms grabbed from ../../checkpoints/afpo_run0_350.p
...
count = 0
count_2 = 0
start_pts =  [[100, 40, 0], 'x', [40, 100, 0], 'x', [40, 40, 0], 'x', [100, 100, 0], 'x']
for swarm in [swarm_1, swarm_2, swarm_3, swarm_4, swarm_5, swarm_6, swarm_7, swarm_8]:
  ...
  for bot in swarm:
    if bot == "red" or bot =="green" or bot == "blue" or bot == "purple":
        continue
    count_2+=1 
    pat = "evolved_swarm_1_18_individual_bots/swarm_{}".format(count)
    path = pat + "/bot_{}".format(count_2)
    os.makedirs(pat, exist_ok=True)
    os.makedirs(path, exist_ok=True)
    vxa.write(path + "/base.vxa")
    ...
    vxd = VXD()
    vxd.set_vxd_tags(body = world, cilia = ciliaWorld, record_voxels=0, record_history=1)
    vxd.set_data(world, ciliaWorld)
    vxd.write(path +"/bot_{}.vxd".format(count_2))
